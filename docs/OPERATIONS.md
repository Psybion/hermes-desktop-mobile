...
Rerunning `./scripts/install.sh` prepares and builds a fresh pinned candidate without modifying the active source tree. After configuration validation, the installer atomically swaps the candidate into the stable source path, restarts the services, and confirms the authenticated gateway baseline. The previous source is deleted only after success; a failed activation stops both managed services, restores the previous tree, and attempts to restart it. The existing gateway token is preserved.
