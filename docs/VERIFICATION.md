...
This record describes the checks run before the initial repository publication. CI repeats the source-level checks from a clean pinned checkout.
...
- no services loaded or started: confirmed
- installer ownership markers, transactional source replacement/rollback, and guarded purge behavior: passed

## Live-runtime acceptance boundary

The initial public artifact was exercised with `./scripts/install.sh --no-start`; it was not activated as a live service during publication review. This record therefore makes no claim that a currently running host gateway represents the exact release candidate.

After installation, `./scripts/verify.sh` is the required live acceptance command. It verifies the managed units, Caddy configuration, active services, authenticated baseline/capabilities, renderer/backend reachability, and Playwright browser QA. Missing Playwright dependencies are a verification failure rather than a skipped success. The optional `npm run qa:chat` lane consumes a configured model turn and remains an explicit operator action.
